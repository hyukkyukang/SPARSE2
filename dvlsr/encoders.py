...
class ColbertEncoder:
    """ColBERTv2 token embeddings (128-d, L2-normalised) — Pilot A reference ceiling."""

    def __init__(self, device="cuda", dtype=torch.float16):
        from transformers import AutoTokenizer, AutoModel
        import torch.nn as nn
        self.tok = AutoTokenizer.from_pretrained(paths.COLBERT)
        self.model = AutoModel.from_pretrained(paths.COLBERT, dtype=dtype).to(device).eval()
        self.device = device
        self.dim = 128
        # ColBERTv2's linear projection ships in the checkpoint as `linear.weight`
        from huggingface_hub import hf_hub_download
        import safetensors.torch as st
        try:
            p = hf_hub_download(paths.COLBERT, "model.safetensors")
            sd = st.load_file(p)
        except Exception:
            p = hf_hub_download(paths.COLBERT, "pytorch_model.bin")
        ...
        self.marker = {"q": "[unused0]", "d": "[unused1]"}

    @torch.no_grad()
    def encode_word_units(self, texts, is_query=False, maxlen=paths.MAXLEN_DOC, keep=None):
        mark = self.marker["q" if is_query else "d"]
        full = [f"{mark} {t}" for t in texts]
        enc = self.tok(full, padding=True, truncation=True, max_length=maxlen,
        ...
        if keep is not None:
            ok &= np.array([keep(w.lower()) if o else False for w, o in zip(words, ok)], bool)
        if not ok.any():
            return WordUnits(np.zeros(0, np.int32), [], torch.zeros(0, 1, self.dim))
        enc = {k: v.to(self.device) for k, v in enc.items()}
        h = self.model(**enc).last_hidden_state
        h = self.proj(h).reshape(-1, self.dim)
        gid_t = torch.from_numpy(gid.astype(np.int64)).to(self.device)
        idx_t = torch.from_numpy(idx.astype(np.int64)).to(self.device)
        ...
        acc /= cnt.unsqueeze(1)
        acc = torch.nn.functional.normalize(acc, dim=-1)          # ColBERT normalises tokens
        sel = np.flatnonzero(ok)
